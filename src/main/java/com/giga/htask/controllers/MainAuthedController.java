...
public class MainAuthedController extends AnchorPane implements Initializable {
    ...
    public void addTab(String title, String fxml, ContentController contentController, Boolean isCloseable) {

        try {
            FXMLLoader fxmlLoader = App.loadLoader(fxml);
            Tab singleTab = new Tab(title, fxmlLoader.load());
            singleTab.setClosable(isCloseable);
            contentController.setCurrentTab(singleTab);
            contentController.setTabPane(contentTabPane);
            AnchorPane anchorPane = (AnchorPane) App.loadViewController(fxml, contentController);
